...
import { getPositions } from 'apiSdk/positions';
...
import { PositionListPage as ShowCaseList } from 'pages/positions';

function DashboardPage() {
  const { session } = useSession();

  const positionStat = useSWR<PaginatedInterface<PositionInterface>>(
    () => `/positions`,
    () => getPositions({ limit: 1, offset: 0 }),
  );
  ...
  const brokerageStat = useSWR<PaginatedInterface<BrokerageInterface>>(
    () => `/brokerages`,
    () => getBrokerages({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Positions',
      result: positionStat?.data?.totalCount,
    ...
    {
      title: 'Brokerages',
      result: brokerageStat?.data?.totalCount,
      isLoading: brokerageStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`positions`} passHref legacyBehavior>
                  <Button
